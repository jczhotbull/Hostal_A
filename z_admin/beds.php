...
$alerta_principal = "0";   // usado para que aparezca alguna nota al ingresar en esta pagina.
    ...
    $query_String_bedsMM = "SELECT COUNT(*) AS total_bedsMM FROM tb_rooms_beds
                             where id_hostel = '$mi_hostel_select'";
    $query_bedsMM = mysqli_query($enlace, $query_String_bedsMM);
    $row_bedsMM = mysqli_fetch_object($query_bedsMM);
...
<i class="fa-solid fa-bed fa-lg "></i> &nbsp; &nbsp;<b><?php echo $row_bedsMM->total_bedsMM; ?></b> Beds
                </div>      
...
<div class="col-xl-3 col-sm-6 col-6 mb-3" <?php if ( $row_bedsMM->total_bedsMM =='0' or $totalRows_the_currencys_out =='0'  ){?>style="display:none"<?php } ?> >
  <div class="card text-white relleno-grama o-hidden h-100">
    <div class="card-body">
      <div class="card-body-icon">
        <i class="fa-solid fa-cash-register fa-xs"  ></i>  
      </div>
      <div class="mr-5 cantidadzzzpe">Beds</div>  
      <div class="infozzz">prices.</div>
    </div>
    <a class=" card-footer card-footerz text-white clearfix small z-1" href="beds_prices.php">
...
<h4 class="glowwhite mt-4" <?php if ( $row_bedsMM->total_bedsMM =='0' ){?>style="display:none"<?php } ?> >Guests in beds:</h4>



 <!-- Icon Cards-->
      
 <div class="row" <?php if ( $row_bedsMM->total_bedsMM =='0' ){?>style="display:none"<?php } ?> >
...
$queryFHL_conteo_t_up = "SELECT id_room_kind FROM tb_rooms_beds
where id_room_kind = '$este_id_kind'
and id_hostel = '$mi_hostel_select'
 order by id_room_kind asc ";

$rooms_conteo_t_up = mysqli_query($enlace, $queryFHL_conteo_t_up) or die(mysqli_error());
$row_rooms_conteo_t_up = mysqli_fetch_assoc($rooms_conteo_t_up);
$totalRows_rooms_conteo_t_up = mysqli_num_rows($rooms_conteo_t_up);
...
0/<?php  echo $totalRows_rooms_conteo_t_up; ?>          
              </div>
...
href="view_guests_bed.php?idtabla=<?php  echo $este_id_kind; ?>&ttitulo=<?php  echo $mi_name_kind; ?>">  
...
<h4 class="glowwhite mt-4" <?php if ( $row_bedsMM->total_bedsMM =='0' ){?>style="display:none"<?php } ?> >
Incidents in Beds by Room Kind:</h4>  
...
$queryFHL_conteo_t = "SELECT id_room_kind FROM tb_rooms_beds
where id_room_kind = '$este_id_kind'
and id_hostel = '$mi_hostel_select'
 order by id_room_kind asc ";
...
$queryFHL_conteo_t_o = "SELECT id_hostel, id_room_kind, note FROM tb_rooms_beds
where id_room_kind = '$este_id_kind'
and id_hostel = '$mi_hostel_select'
and note !='' order by id_room_kind asc ";
...
<?php echo $totalRows_rooms_conteo_t_o;  ?>/<?php echo $totalRows_rooms_conteo_t;  ?>
...
href="view_beds_inc.php?idtabla=<?php  echo $este_id_kind; ?>&ttitulo=<?php  echo $mi_name_kind; ?>">  
