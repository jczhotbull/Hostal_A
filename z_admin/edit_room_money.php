...
if(isset($_POST['add_price']))  // chequea si se ha enviado algo, de ser si --> se conecta a la BD y comprueba
{
    ...
    $query_price_room = "INSERT INTO tb_prices_rooms(id_hostel, id_room_kind, name_prices_rooms,
    taxes_room, discount_room, prices_set_by_who) 
...
$queryDiss = "DELETE FROM tb_prices_rooms WHERE id_prices_rooms = '$id_price_borrar' LIMIT 1";
...
if (!mysqli_query($enlace,$queryDiss))      // si no ha logrado borrar los direcc del hostel
 ...
 <button type="button" class="btn btn-dark btn-lg btn-block" style="margin-top:1px;"  onClick="javascript:history.go(-1)" ><i class="fa-solid fa-arrow-right-from-bracket fa-flip-horizontal fa-lg"></i></button>
                ...
                <div class="alert col-md-3 col-lg-3 alert-primary" role="alert">
                    <i class="fa-solid fa-money-bill-trend-up fa-lg "></i> &nbsp; &nbsp;  <i> <b><?php echo $ttitulo ?></b> Rooms.</i>
                </div> 
...
<form  method="POST"  name="add_price">                           
                            ...
                            <div class="form-row">  <!-- datos principales -->
                                ...
                                <span class="text-info"> Set New Price for <b><?php echo $ttitulo ?></b> Rooms: </span>            
...
$queryFHL_pp = "SELECT * FROM tb_prices_rooms, taxes, discounts
WHERE tb_prices_rooms.id_hostel = '$mi_hostel_select' 
and   tb_prices_rooms.id_room_kind = '$idtbla'
and   tb_prices_rooms.taxes_room = taxes.id_taxes
and   tb_prices_rooms.discount_room = discounts.id_discounts
ORDER BY tb_prices_rooms.set_prices_date desc limit 5";
...
<?php echo $row_room_pp['set_prices_date']; ?>
</td>


<td class="align-middle" align="center">
<?php echo $row_room_pp['name_prices_rooms']; ?> - <?php echo $row_the_currencys_prim['symbol_currency']; ?>
</td>
...
<div data-toggle="tooltip" data-placement="top" title="Delete Price." >
  <button type="button" class="btn btn-outline-danger btn-sm mb-1" data-toggle="modal"
                  data-target="#borrar<?php echo $row_room_pp['id_prices_rooms'];  ?>">
...
<?php include("deletes/delete_room_price_modal.php"); ?>
